...
class LinkConfigError(ValueError):
    """
    Raised when a link cannot be safely built because a required
    environment variable (e.g. the public host/port for a raw TCP
    proxy) is missing or empty. Callers must surface this as a clear
    error to the user instead of falling back to a broken link.
    """
...
def generate_vless_link(uid, remark="SE7O", address=None, extra=None):
    ...
    if transport == "tcp":
        ...
        # This transport only works if the host platform has actually
        # exposed raw_tcp_port to the internet (see raw_tcp.py). Never
        # silently fall back to the internal port/domain here — that
        # produces a link that looks valid but can never connect.
        if not settings.raw_tcp_enabled:
            raise LinkConfigError(
                "Raw TCP transport is disabled (RAW_TCP_ENABLED is not set to true)."
            )
        if not settings.raw_tcp_public_host or not settings.raw_tcp_public_port:
            raise LinkConfigError(
                "Raw TCP transport requires RAW_TCP_PUBLIC_HOST and "
                "RAW_TCP_PUBLIC_PORT to be set (the public host/port that "
                "the platform's TCP proxy exposes). Configure a TCP Proxy "
                "for this port on your host and set those variables, then "
                "try again."
            )
        addr = settings.raw_tcp_public_host
        port = int(settings.raw_tcp_public_port)
