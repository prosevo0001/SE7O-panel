    ...
    <div style="background:var(--surface2);border:1px solid var(--border2);border-radius:28px;padding:48px 40px;width:100%;max-width:400px;box-shadow:0 0 40px var(--primary-dim);backdrop-filter:blur(20px);">
      <div style="text-align:center;margin-bottom:32px;">
        ...
        <div style="font-family:'Orbitron',sans-serif;font-size:1.5rem;font-weight:900;color:var(--primary);margin-top:12px;display:flex;align-items:center;justify-content:center;gap:8px;">
          SE7O-SNA Panel <span style="font-size:0.8rem; font-family:'Inter'; color:var(--bg); background:var(--primary); padding:2px 6px; border-radius:4px;">V 3.0.3</span>
        </div>
  ...
  <header class="header">
    <div class="header-inner">
      <div style="display:flex;align-items:center;gap:16px;">
        <img src="/img/logo.png" alt="SE7O-SNA" style="height:32px;width:32px;border-radius:50%;vertical-align:middle;"> <span class="logo">SE7O-SNA</span><span class="version-tag">v3.0.3</span>
...
function renderLinks(links){
  ...
  tb.innerHTML=links.map(l=>{
    ...
    const linkErr=l.link_error||'',copyLink=linkErr?`toast(${JSON.stringify(linkErr).replace(/"/g,'&quot;')},true)`:`cpLink('${esc(l.vless_link)}')`,qrLink=linkErr?`toast(${JSON.stringify(linkErr).replace(/"/g,'&quot;')},true)`:`showQR('${esc(l.vless_link)}')`;
    return`<tr>
      <td><input type="checkbox" value="${l.uuid}" ${check} onchange="toggleSelectUid('${l.uuid}')"></td>
      <td style="font-weight:600">${labelDisplay}</td>
      <td><span class="tag tag-vless">VLESS</span> <span class="tag" style="opacity:.75">${(l.transport||'ws').toUpperCase()}</span>${linkErr?` <span class="tag tag-off" title="${esc(linkErr)}">⚠ ${t('config_error')||'Config error'}</span>`:''}</td>
        ...
        <div style="display:flex; flex-direction:column; gap:6px; align-items:center;">
          ...
          <div style="display:flex; flex-wrap:wrap; gap:4px; justify-content:center;">
            ${l.label === 'This Server is Free' ? `
              <button class="act-btn act-copy" title="${t('copy')}" onclick="${copyLink}"><svg class="ic"><use href="#ic-copy"/></svg></button>
              <button class="act-btn act-sub" title="${t('sub')}" onclick="cpSub('${l.uuid}')"><svg class="ic"><use href="#ic-link"/></svg></button>
              <button class="act-btn act-qr" title="${t('qr')}" onclick="${qrLink}"><svg class="ic"><use href="#ic-qr"/></svg></button>
            ` : `
