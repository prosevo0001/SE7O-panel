...
from app.vless import generate_vless_link, LinkConfigError
...
async def create_link(request: Request, _=Depends(require_auth)):
    ...
    extra = {"custom_path": custom_path, "custom_sni": custom_sni, "custom_host": custom_host,
              "custom_fp": custom_fp, "fragment": fragment, "transport": transport}
    try:
        # Validate the link is actually buildable (e.g. raw-TCP requires
        # RAW_TCP_PUBLIC_HOST/PORT) before writing anything to the DB.
        generate_vless_link(uid, remark=f"SE7O-{label}", extra=extra)
...
async def update_link(uid: str, request: Request, _=Depends(require_auth)):
    ...
    if "transport" in body:
        new_transport = _normalize_transport(body.get("transport"))
        if new_transport == "tcp":
            merged = {**link, **updates}
            try:
                generate_vless_link(uid, remark="SE7O-check", extra={
                    "custom_path": merged.get("custom_path", ""),
                    "custom_sni": merged.get("custom_sni", ""),
                    "custom_host": merged.get("custom_host", ""),
                    "custom_fp": merged.get("custom_fp", "chrome"),
                    "fragment": merged.get("fragment", ""),
                    "transport": "tcp",
                })
            except LinkConfigError as e:
                raise HTTPException(status_code=400, detail=str(e))
